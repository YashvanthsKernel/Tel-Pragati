...
export function WellboreScene({
  ...
  useEffect(() => {
    if (!containerRef.current || !webGlSupported || showFallback) return;
    ...
    const dirLight2 = new THREE.DirectionalLight(0x197f8c, 0.6);
    ...
    let rodColor = 0x197f8c;
    let wellboreColor = 0xaeb8c2;
    ...
    const groundMat = new THREE.MeshStandardMaterial({ color: 0xeef2f5, roughness: 0.8 });
    ...
    const postMat = new THREE.MeshStandardMaterial({ color: 0xaeb8c2 });
    ...
    const horseMat = new THREE.MeshStandardMaterial({ color: 0x197f8c });
    ...
    const rulerLineMat = new THREE.LineBasicMaterial({ color: 0x74808b });
    ...
    <div className="relative w-full h-full min-h-[420px] bg-surface-0 rounded-lg border border-line overflow-hidden select-none flex flex-col justify-between">
      ...
      {interactive && (
        <div className="absolute top-3 left-3 right-3 flex flex-wrap items-center justify-between gap-2 z-10 pointer-events-none">
          ...
          <div className="flex items-center gap-1 bg-surface-1/90 backdrop-blur-md p-1 rounded-md border border-line shadow-popup pointer-events-auto overflow-x-auto max-w-full">
            {(
              [
              ...
              <button
                ...
                className={`px-2 py-1 rounded text-[10px] font-mono uppercase font-bold transition-colors whitespace-nowrap ${
                  overlayMode === m.id
          ...
          <div className="flex items-center gap-1 bg-surface-1/90 backdrop-blur-md p-1 rounded-md border border-line shadow-popup pointer-events-auto">
            {(
              [
              ...
              <button
                ...
                className={`px-2 py-1 rounded text-[10px] font-mono transition-colors ${
                  cameraBookmark === b.id
                    ? "bg-surface-2 text-text-primary font-bold"
            ...
            <button
              ...
              className={`p-1 rounded text-[10px] font-mono flex items-center gap-1 border transition-colors ${
                isCrossSection
      ...
      <div className="absolute bottom-3 left-3 right-3 bg-surface-1/90 backdrop-blur-md p-2.5 rounded-md border border-line shadow-popup flex items-center justify-between z-10 pointer-events-none text-xs font-mono">
        <div className="flex items-center gap-4">
          <div>
            <span className="text-[10px] text-text-muted">BHT:</span>
            <span className="font-bold text-accent-thermal ml-1">{wellState.inferred.bottomholeTempC.value}°C</span>
          </div>
          <div>
            <span className="text-[10px] text-text-muted">Viscosity:</span>
            <span className="font-bold text-accent-mechanical ml-1">{wellState.inferred.viscosityCp.value.toLocaleString()} cP</span>
          </div>
          <div>
            <span className="text-[10px] text-text-muted">Rod Floating Risk:</span>
            <span className={`font-bold ml-1 ${wellState.rodFloatingRiskPct > 50 ? "text-status-warn" : "text-status-safe"}`}>
